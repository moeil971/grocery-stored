...
// احراز هویت مشتریان با ایمیل داخلیِ مخفی انجام می‌شود؛ مشتری فقط شماره موبایل خود را می‌بیند.
// هیچ ایمیل واقعی برای مشتری لازم نیست.
const authEmail=phone=>`${normalizePhone(phone)}@accounts.moeili.com`;
...
let products=[],categories=[],cart=JSON.parse(localStorage.getItem("grocery_cart")||"[]"),activeFilter="all",activeCategory="",currentUser=null,profile=null;
...
document.addEventListener("DOMContentLoaded",async()=>{
  ...
  renderCart();
  sb.auth.onAuthStateChange(async(_e,session)=>{currentUser=session?.user||null;if(currentUser)await loadProfile();else profile=null;renderAccountView();if(currentUser)loadMyOrders()});
});
...
function bindUI(){
  ...
  $("#checkoutBtn").onclick=()=>{if(!cart.length)return toast("سبد خرید خالی است");if(!currentUser){closeCart();openAccountSheet("برای ثبت سفارش ابتدا وارد حساب شوید.");return}if(profile)loadProfile();$("#checkoutModal").classList.add("open");renderDeliveryOptions();renderCheckoutSummary()};
  ...
  $("#navOrders").onclick=()=>{closeAllSheets();openAccountSheet(currentUser?"":"orders");};
  ...
  $("#hasAccountBtn").onclick=()=>showAuth("login");
  $("#noAccountBtn").onclick=()=>showAuth("signup");
  $("#backAccountChoice").onclick=()=>showAccountChoice();
  $("#authForm").onsubmit=submitAuth;
  $("#logoutBtn2").onclick=async()=>{await sb.auth.signOut();toast("از حساب خارج شدید")};
}
function closeAllSheets(){$("#categorySheet").classList.remove("open");$("#accountSheet").classList.remove("open");closeCart()}

async function refreshSession(){const {data:{session}}=await sb.auth.getSession();currentUser=session?.user||null;if(currentUser)await loadProfile();renderAccountView()}
...
async function loadProfile(){
 ...
 if(profile){
   ...
   $("#checkoutName").value=profile.full_name||"";
   $("#checkoutPhone").value=profile.phone||"";
   $("#checkoutAddress").value=profile.address||"";
 }
}
async function saveProfile(e){
 e.preventDefault();if(!currentUser)return;
 const fd=new FormData(e.target),msg=$("#profileMessage");msg.textContent="در حال ذخیره...";
 const payload={user_id:currentUser.id,full_name:String(fd.get("full_name")).trim(),phone:normalizePhone(fd.get("phone")),address:String(fd.get("address")).trim(),updated_at:new Date().toISOString()};
 const {data,error}=await sb.from("profiles").upsert(payload).select().single();
 if(error){msg.textContent=error.message;return} profile=data;renderAccountView();msg.textContent="اطلاعات حساب ذخیره شد.";
}
...
async function submitOrder(e){
 ...
 if(!currentUser){$("#checkoutMessage").textContent="ابتدا وارد حساب کاربری شوید.";return}
 const fd=new FormData(e.target),msg=$("#checkoutMessage");msg.textContent="در حال ثبت سفارش...";
 const payload={customer_name:String(fd.get("name")).trim(),phone:normalizePhone(fd.get("phone")),address:String(fd.get("address")).trim(),note:String(fd.get("note")||"").trim(),delivery_method:String(fd.get("delivery_method")||"pickup"),items:cart.map(x=>({product_id:x.id,quantity:x.quantity}))};
 ...
 cart=[];saveCart();closeCart();$("#checkoutModal").classList.remove("open");loadMyOrders();location.href="invoice.html?order="+encodeURIComponent(data.order_number);
}

/* ===== حساب کاربری ===== */
let authMode="login";
function openAccountSheet(message){
 closeAllSheets();$("#accountSheet").classList.add("open");
 if(currentUser){renderAccountView();if(message==="orders")setTimeout(()=>document.querySelector("#myOrders")?.scrollIntoView({behavior:"smooth",block:"start"}),150);return}
 showAccountChoice();
}
...
function toggleAuthMode(){authMode=authMode==="login"?"signup":"login";renderAuthForm()}
function renderAuthForm(){
 $("#authTitle").textContent=authMode==="login"?"ورود به حساب":"ساخت حساب جدید";
 $("#authSubmitBtn").textContent=authMode==="login"?"ورود":"ساخت حساب";
 $("#authToggle").textContent=authMode==="login"?"حساب ندارید؟ ثبت‌نام کنید":"حساب دارید؟ وارد شوید";
 $("#signupFields").classList.toggle("hidden",authMode==="login");
 $("#authPassword").value="";
 $("#authMessage").textContent="";
}
async function submitAuth(e){
 e.preventDefault();
 const fd=new FormData(e.target),msg=$("#authMessage");
 const phone=normalizePhone(fd.get("phone")),password=String(fd.get("password")||"");
 ...
 if(authMode==="signup") {
   const full_name=String(fd.get("full_name")||"").trim(),address=String(fd.get("address")||"").trim();
   if(full_name.length<2||address.length<5){msg.textContent="نام و آدرس دقیق را کامل کنید.";return}
   // Supabase Phone Auth نیاز به SMS provider دارد؛ برای فروشگاه شما از Email Auth با ایمیل داخلی و مخفی استفاده می‌کنیم.
   const internalEmail=authEmail(phone);
   const {data,error}=await sb.auth.signUp({email:internalEmail,password,options:{data:{full_name,phone,address}}});
   if(error){console.error(error);msg.textContent=error.message?.toLowerCase().includes("already")||error.status===422?"این شماره قبلاً ثبت شده است.":"ثبت‌نام انجام نشد: "+(error.message||"خطای نامشخص");return}
   if(data.session&&data.user){
     currentUser=data.user;
     const {error:pe}=await sb.from("profiles").upsert({user_id:data.user.id,full_name,phone,address}).select().single();
     if(pe){msg.textContent="حساب ساخته شد اما ذخیره اطلاعات پروفایل با خطا مواجه شد: "+pe.message;return}
     await loadProfile();renderAccountView();toast("حساب شما با موفقیت ساخته شد");
   } else {
     msg.textContent="حساب ساخته شد، اما ورود خودکار انجام نشد. در Supabase گزینه Confirm email باید خاموش باشد.";
   }
 } else {
   const {data,error}=await sb.auth.signInWithPassword({email:authEmail(phone),password});
   if(error){console.error(error);msg.textContent="شماره موبایل یا رمز عبور اشتباه است.";return}
   currentUser=data.user;await loadProfile();renderAccountView();toast("خوش آمدید");
 }
}
function renderAccountView(){
 if(currentUser){
  $("#authView").classList.add("hidden");$("#profileView").classList.remove("hidden");
  $("#accountName").textContent=profile?.full_name||"مشتری فروشگاه معیلی";
  $("#accountPhone").textContent=profile?.phone||"";
  if($("#profileName"))$("#profileName").value=profile?.full_name||"";
  if($("#profilePhone"))$("#profilePhone").value=profile?.phone||"";
  if($("#profileAddress"))$("#profileAddress").value=profile?.address||"";
  loadMyOrders();
 }else{
  $("#authView").classList.remove("hidden");$("#profileView").classList.add("hidden");renderAuthForm();
 }
 $("#navAccountDot").classList.toggle("hidden",!currentUser);
}
async function loadMyOrders(){
 if(!currentUser||CONFIG.SUPABASE_URL.includes("YOUR_"))return;
 const {data,error}=await sb.rpc("get_my_orders");
 const box=$("#myOrders");
 if(error||!data?.length){box.innerHTML='<div class="empty-state">هنوز سفارشی ثبت نکرده‌اید.</div>';return}

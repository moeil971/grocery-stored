...
async function submitAuth(e){
 ...
 if(authMode==="signup") {
   ...
   const {data,error}=await sb.auth.signUp({phone,password,options:{data:{full_name,phone,address}}});
   if(error){console.error(error);msg.textContent=error.message?.includes("already")?"این شماره قبلاً ثبت شده است.":"ثبت‌نام انجام نشد: "+(error.message||"خطای نامشخص");return}
   if(data.session&&data.user){
     ...
     msg.textContent="حساب ساخته شد. اگر تأیید شماره موبایل فعال باشد، کد تأیید برای شما ارسال می‌شود.";
   }
 } else {

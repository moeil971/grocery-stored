...
function renderProducts(){
 ...
 if(more){more.classList.toggle("hidden", visibleProductCount>=total); $("#productsCount").textContent=total>visibleProductCount?`${num(visibleProductCount)} از ${num(total)} محصول نمایش داده می‌شود`:total?`${num(total)} محصول`:"";}
...
let productFeedObserver;
function setupProductFeed(){
  const sentinel=$("#productsFeedSentinel"); if(!sentinel || !("IntersectionObserver" in window)) return;
  productFeedObserver?.disconnect();
  productFeedObserver=new IntersectionObserver(entries=>{ if(entries[0].isIntersecting){ const q=($("#searchInput")?.value||"").trim().toLowerCase(); let list=products.filter(p=>(p.name||"").toLowerCase().includes(q)); if(activeFilter==="discount")list=list.filter(p=>discount(p.consumer_price,p.store_price)>0); if(activeFilter==="popular")list=list.filter(p=>p.is_popular); if(activeFilter==="new")list=list.filter(p=>p.is_new); if(activeCategory)list=list.filter(p=>(p.category_ids||[]).includes(activeCategory)); if(visibleProductCount<list.length){visibleProductCount+=15;renderProducts();}} },{rootMargin:"500px 0px"});
  productFeedObserver.observe(sentinel);
}

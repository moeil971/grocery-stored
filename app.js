...
function renderAuthForm(){
 ...
 $("#authSubmitBtn").textContent=authMode==="login"?"ورود":"ثبت‌نام";
 $("#authToggle").textContent=authMode==="login"?"حساب ندارید؟ ثبت‌نام کنید":"حساب دارید؟ وارد شوید";$("#signupFields").classList.toggle("hidden",authMode==="login");
 $("#authMessage").textContent="";
}
async function submitAuth(e){
 e.preventDefault();const fd=new FormData(e.target),msg=$("#authMessage");
 const phone=normalizePhone(fd.get("phone")),password=String(fd.get("password")||"");
 if(!/^09\\d{9}$/.test(phone)){msg.textContent="شماره موبایل باید با 09 شروع شود.";return}
 msg.textContent="در حال پردازش...";
 const email=`${phone}@moeili.local`;
 if(authMode==="signup"){
   ...
   if(error){msg.textContent=error.message.includes("already")?"این شماره قبلاً ثبت شده است.":"ثبت‌نام انجام نشد.";return}
   if(data.user)await sb.from("profiles").upsert({user_id:data.user.id,full_name,phone,address});
   if(data.session){currentUser=data.user;await loadProfile();renderAccountView();toast("حساب شما ساخته شد");}
   else msg.textContent="حساب ساخته شد. برای ورود، تأیید ایمیل را در تنظیمات Supabase خاموش کنید.";
 }else{
   const {data,error}=await sb.auth.signInWithPassword({email,password});
   if(error){msg.textContent="شماره موبایل یا رمز عبور اشتباه است.";return}
   currentUser=data.user;await loadProfile();$("#accountSheet").classList.remove("open");toast("خوش آمدید");
 }
}

...
let products=[],categories=[],cart=JSON.parse(localStorage.getItem("grocery_cart")||"[]"),activeFilter="all",activeCategory="",visibleProductCount=15;
let siteSettings={banner_url:"",logo_url:"",shipping_enabled:true,shipping_cost:0,minimum_order_amount:0};
...
function bindUI(){
  $("#searchInput").addEventListener("input",()=>{visibleProductCount=15;renderProducts();});
  const allBtn=$("#showAllProducts"), popularBtn=$("#showPopularProducts");
  allBtn?.addEventListener("click",()=>{activeFilter="all";activeCategory="";visibleProductCount=15;setProductsView("all");renderCategoryChips();renderProducts()});
  popularBtn?.addEventListener("click",()=>{
    const c=findPopularCategory();
    if(!c){toast("ابتدا دسته‌بندی «پرفروش‌ها» را از پنل مدیریت اضافه کنید.");return}
    activeFilter="all";activeCategory=c.id;visibleProductCount=15;setProductsView("popular");renderCategoryChips();renderProducts();
  });
  ...
  // دسته‌بندی‌ها مستقیماً زیر بخش محصولات نمایش داده می‌شوند؛ پنجره جداگانه حذف شده است.
  $("#loadMoreProducts")?.addEventListener("click",()=>{visibleProductCount+=15;renderProducts();});
  $("#closeTrackingSheet").onclick=()=>$("#trackingSheet").classList.remove("open");
  $("#trackingForm").onsubmit=trackOrder;
}
function closeAllSheets(){$("#trackingSheet").classList.remove("open");closeCart()}
...
function normalizeFa(s){return String(s||"").replace(/[‌‌\s]/g,"").replace(/ي/g,"ی").replace(/ك/g,"ک").replace(/ة/g,"ه").toLowerCase()}
function findPopularCategory(){return categories.find(c=>{const n=normalizeFa(c.name);return n.includes("پرفروش")||n.includes("پرفروشها")||n.includes("پرفروشها")})||null}
function setProductsView(view){document.querySelectorAll(".view-switch").forEach(x=>x.classList.remove("active"));$(view==="popular"?"#showPopularProducts":"#showAllProducts")?.classList.add("active")}
function renderCategoryChips(){
  const wrap=$("#categoryChips");
  if(!wrap)return;
  const chip=(id,name,color,active)=>`<button class="cat-chip ${active?"active":""}" data-cat="${id}" style="--c:${color||"#16A863"}"><span class="cat-dot" aria-hidden="true"></span><span class="cat-name">${escapeHtml(name)}</span></button>`;
  ...
  wrap.innerHTML=all+list||'<div class="empty-state">هنوز دسته‌بندی‌ای ثبت نشده.</div>';
  document.querySelectorAll("[data-cat]").forEach(b=>b.onclick=()=>{activeCategory=b.dataset.cat;activeFilter="all";setProductsView("all");renderCategoryChips();visibleProductCount=15;renderProducts()});
}
...
function hexToRgb(hex){const h=String(hex||"").replace("#","");if(!/^[0-9a-fA-F]{6}$/.test(h))return null;return {r:parseInt(h.slice(0,2),16),g:parseInt(h.slice(2,4),16),b:parseInt(h.slice(4,6),16)}}
function mixHex(hex,amount,target=255){const c=hexToRgb(hex);if(!c)return hex;const f=amount/100;return "#"+[c.r,c.g,c.b].map(v=>Math.round(v+(target-v)*f).toString(16).padStart(2,"0")).join("")}
function renderSettings(){
 ...
 const primary=siteSettings.theme_primary||"#FF5A1F", bg=siteSettings.theme_bg||"#F7F7F8";
 document.documentElement.style.setProperty("--primary",primary);
 document.documentElement.style.setProperty("--primary-soft",mixHex(primary,88));
 document.documentElement.style.setProperty("--primary-deep",mixHex(primary,-12,0));
 document.documentElement.style.setProperty("--bg",bg);
 document.documentElement.style.setProperty("--page-glow",mixHex(primary,94));
...
async function loadProducts(){
  ...
  const {data,error}=await sb.from("products").select("*, product_categories(category_id)").eq("is_visible",true).order("sort_order",{ascending:true,nullsFirst:false}).order("created_at",{ascending:false});
...
function renderProducts(){
 ...
 if(activeFilter==="popular") list=list.filter(p=>p.is_popular===true);
 ...
 list=list.slice(0,visibleProductCount);
 ...
 if(more){
   ...
   more.style.display=hasMore ? "grid" : "none";
   $("#productsCount").textContent=hasMore ? `${num(visibleProductCount)} از ${num(total)} محصول نمایش داده می‌شود` : total ? `${num(total)} محصول` : "";
 }
 document.querySelectorAll(".add-btn").forEach(b=>b.onclick=e=>{e.stopPropagation();addToCart(b.dataset.id)});
 document.querySelectorAll(".product-plus").forEach(b=>b.onclick=e=>{e.stopPropagation();changeQty(b.dataset.id,1)});
 document.querySelectorAll(".product-minus").forEach(b=>b.onclick=e=>{e.stopPropagation();changeQty(b.dataset.id,-1)});
  setupProductFeed();
}
function card(p){
 ...
 const img=p.image_url?`<img src="${escapeAttr(p.image_url)}" alt="${escapeAttr(p.name)}" loading="lazy" decoding="async">`:`<div class="placeholder">🧺</div>`;
 return `<article class="product-card" data-id="${escapeAttr(p.id)}"><div class="product-image">${d&&sale&&showBadge?`<span class="discount-badge">${num(d)}٪</span>`:""}${img}</div>
...
function openProductDetail(id){
 ...
 const d=discount(p.consumer_price,p.store_price),sale=p.store_price_enabled!==false,out=Number(p.stock)<=0;
 const image=p.image_url?`<img src="${escapeAttr(p.image_url)}" alt="${escapeAttr(p.name)}" loading="lazy" decoding="async">`:`<div class="detail-placeholder">🧺</div>`;
 $("#productDetailImage").innerHTML=`${d&&sale&&p.show_discount_badge!==false?`<span class="discount-badge detail-badge">${num(d)}٪</span>`:""}${image}`;
 $("#productDetailName").textContent=p.name||"محصول";
 $("#productDetailStock").textContent=out?"ناموجود":`موجودی: ${num(p.stock)} ${p.unit||"عدد"}`;
 $("#productDetailPrice").innerHTML=`${sale&&d?`<span class="consumer">${money(p.consumer_price)}</span>`:""}<strong>${money(sale?p.store_price:p.consumer_price)}</strong><small>/ ${escapeHtml(p.unit||"عدد")}</small>`;
 $("#productDetailDescription").textContent=(p.description||"").trim()||"توضیحی برای این محصول ثبت نشده است.";
 const row=cart.find(x=>String(x.id)===String(p.id));
 $("#productDetailActions").innerHTML=row?`<div class="detail-qty"><button type="button" id="detailMinus">−</button><span>${num(row.quantity)}</span><button type="button" id="detailPlus">+</button></div><div class="added-note">به سبد خرید اضافه شد</div>`:`<button class="primary-btn full" id="detailAdd" ${out?"disabled":""}>${out?"ناموجود":"افزودن به سبد خرید"}</button>`;
 if(row){$("#detailMinus").onclick=()=>{changeQty(p.id,-1);openProductDetail(p.id)};$("#detailPlus").onclick=()=>{changeQty(p.id,1);openProductDetail(p.id)}}
 else if(!out)$("#detailAdd").onclick=()=>{addToCart(p.id);openProductDetail(p.id)};
 $("#productDetailSheet").classList.add("open"); document.body.classList.add("no-scroll");
}
function closeProductDetail(){$("#productDetailSheet")?.classList.remove("open");document.body.classList.remove("no-scroll")}
...
function addToCart(id){
 ...
 saveCart();renderCart();updateProductCard(p.id);animateAddedCard(p.id);toast(`${p.name} به سبد خرید اضافه شد`);
}
function changeQty(id,delta){const r=cart.find(x=>String(x.id)===String(id));if(!r)return;const p=products.find(x=>String(x.id)===String(id));const max=Number(p?.stock??r.stock);r.quantity=Math.max(0,Math.min(max,r.quantity+delta));if(!r.quantity)cart=cart.filter(x=>String(x.id)!==String(id));saveCart();renderCart();updateProductCard(id)}

function updateProductCard(id){
 const cardEl=document.querySelector(`.product-card[data-id="${CSS.escape(String(id))}"]`);
 const p=products.find(x=>String(x.id)===String(id));
 if(!cardEl||!p)return;
 const old=cardEl.outerHTML;
 const d=discount(p.consumer_price,p.store_price),out=Number(p.stock)<=0,sale=p.store_price_enabled!==false,showBadge=p.show_discount_badge!==false;
 const img=p.image_url?`<img src="${escapeAttr(p.image_url)}" alt="${escapeAttr(p.name)}" loading="lazy" decoding="async">`:`<div class="placeholder">🧺</div>`;
 const fresh=document.createElement('div');
 fresh.innerHTML=`${card(p)}`;
 const next=fresh.firstElementChild;
 if(next){cardEl.replaceWith(next);bindSingleProductCard(next);}
}
function bindSingleProductCard(cardEl){
 cardEl.querySelector('.add-btn')?.addEventListener('click',e=>{e.stopPropagation();addToCart(cardEl.dataset.id)});
 cardEl.querySelector('.product-plus')?.addEventListener('click',e=>{e.stopPropagation();changeQty(cardEl.dataset.id,1)});
 cardEl.querySelector('.product-minus')?.addEventListener('click',e=>{e.stopPropagation();changeQty(cardEl.dataset.id,-1)});
}
function animateAddedCard(id){
 requestAnimationFrame(()=>{
  const el=document.querySelector(`.product-card[data-id="${CSS.escape(String(id))}"]`);
  if(!el)return; el.classList.remove('is-added'); void el.offsetWidth; el.classList.add('is-added');
  const btn=el.querySelector('.add-btn'); if(btn){btn.classList.add('is-adding');setTimeout(()=>btn.classList.remove('is-adding'),450);}
 });
}
...
function renderCheckoutSummary(){
 const sub=cart.reduce((a,x)=>a+x.consumer_price*x.quantity,0),final=cart.reduce((a,x)=>a+x.store_price*x.quantity,0),minimum=Math.max(0,Number(siteSettings.minimum_order_amount)||0);
 const minText=minimum?`<div>حداقل مبلغ سفارش: <b>${money(minimum)}</b></div>`:"";
 $("#checkoutSummary").innerHTML=`<div>جمع قیمت مصرف‌کننده: <b>${money(sub)}</b></div><div>مجموع تخفیف: <b>${money(sub-final)}</b></div><div>مبلغ نهایی کالاها: <b>${money(final)}</b></div>${minText}`;
}
async function submitOrder(e){
 e.preventDefault();if(!cart.length)return;
 const minimum=Math.max(0,Number(siteSettings.minimum_order_amount)||0),cartTotal=cart.reduce((a,x)=>a+Number(x.store_price)*Number(x.quantity),0);
 if(minimum>0&&cartTotal<minimum){const msg=$("#checkoutMessage");msg.textContent=`حداقل مبلغ سفارش ${money(minimum)} است. برای ثبت سفارش باید حداقل ${money(minimum-cartTotal)} دیگر به سبد خرید اضافه کنید.`;return}
 const fd=new FormData(e.target),msg=$("#checkoutMessage");
 const phone=normalizePhone(fd.get("phone"));
 if(!isValidIranMobile(phone)){msg.textContent="شماره موبایل معتبر وارد کنید؛ شماره‌های ساختگی یا تکراری پذیرفته نمی‌شوند.";return}
 const name=String(fd.get("name")||"").trim(),address=String(fd.get("address")||"").trim();
 if(!isValidCustomerName(name)){msg.textContent="نام واردشده معتبر نیست؛ نام و نام‌خانوادگی واقعی را وارد کنید.";return}
...
function isValidIranMobile(v){
  v=normalizePhone(v);
  if(!/^09\d{9}$/.test(v)) return false;
  // شماره‌های تکراری/ساختگی رایج مثل 09999999999 پذیرفته نمی‌شوند.
  if(/^0(\d)\1{9}$/.test(v)) return false;
  const local=v.slice(2);
  if(/^([0-9])\1{8}$/.test(local)) return false;
  const knownPrefixes=/^0(90|91|92|93|99)\d{8}$/;
  return knownPrefixes.test(v);
}
function isValidCustomerName(v){
  v=String(v||"").trim().replace(/\s+/g," ");
  if(v.length<3 || v.length>60) return false;
  if(!/[A-Za-zآ-ی]/.test(v)) return false;
  if(/[0-9_@#$%&*+=<>|~`^{}\[\]\\/]/.test(v)) return false;
  if(/(.)\1\1/.test(v.replace(/\s/g,""))) return false;
  return true;
}
...
// نمایش مرحله‌ای محصولات: عمداً خودکار نیست تا با ۱۸/۱۰۰ محصول همه یک‌جا روی صفحه نیایند.
// با «دیدن ادامه محصولات» هر بار ۱۵ محصول دیگر اضافه می‌شود.
function setupProductFeed(){
  const sentinel=$("#productsFeedSentinel");
  if(sentinel) sentinel.setAttribute("aria-hidden","true");
}

document.addEventListener("click",e=>{const card=e.target.closest(".product-card");if(card&&!e.target.closest("button"))openProductDetail(card.dataset.id);});
document.addEventListener("DOMContentLoaded",()=>{document.getElementById("closeProductDetail")?.addEventListener("click",closeProductDetail);document.getElementById("productDetailSheet")?.addEventListener("click",e=>{if(e.target.id==="productDetailSheet")closeProductDetail()});});

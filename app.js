...
async function saveProfile(e){
 ...
 const fd=new FormData(e.target),msg=$("#profileMessage");
 ...
 const payload={user_id:currentUser.id,full_name:String(fd.get("full_name")).trim(),phone,address:String(fd.get("address")).trim(),updated_at:new Date().toISOString()};
...
async function submitOrder(e){
 ...
 const payload={customer_name:String(fd.get("name")).trim(),phone,address:String(fd.get("address")).trim(),note:String(fd.get("note")||"").trim(),delivery_method:String(fd.get("delivery_method")||"pickup"),items:cart.map(x=>({product_id:x.id,quantity:x.quantity}))};
...
function bindPhoneValidation(){
  ["#authForm input[name=phone]","#checkoutPhone","#profilePhone"].forEach(sel=>{

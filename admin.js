...
function bind(){
 ...
 $("#productForm").onsubmit=saveProduct;$("#settingsForm").onsubmit=saveSettings;$("#customersPdfBtn").onclick=printCustomers;$("#ordersPdfBtn").onclick=printOrders;$("#productImage").onchange=previewImage;
...
async function printOrders(){
  if(!allOrders.length){toast("سفارشی برای چاپ وجود ندارد");return}
  ...
  w.document.write(`<html lang="fa" dir="rtl"><head><meta charset="utf-8"><title>تمام سفارش‌های فروشگاه معیلی</title><style>body{font-family:Arial,sans-serif;padding:20px;color:#222}h1{text-align:center;font-size:20px}p{font-size:12px}table{width:100%;border-collapse:collapse;font-size:10px}th,td{border:1px solid #bbb;padding:7px;vertical-align:top;text-align:right}th{background:#eee}.total{font-weight:bold}</style></head><body><h1>تمام سفارش‌های فروشگاه معیلی</h1><p>تعداد سفارش‌ها: ${num(allOrders.length)} — تاریخ تهیه گزارش: ${new Date().toLocaleString("fa-IR")}</p><table><thead><tr><th>شماره سفارش</th><th>تاریخ</th><th>مشتری</th><th>موبایل</th><th>آدرس</th><th>وضعیت</th><th>اقلام</th><th>مبلغ نهایی</th></tr></thead><tbody>${rows}</tbody></table><script>window.onload=()=>window.print()</script></body></html>`);
  w.document.close();
}

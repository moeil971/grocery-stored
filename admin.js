...
async function loadSettings(){const {data,error}=await sb.from("site_settings").select("*");if(!error)(data||[]).forEach(x=>siteSettings[x.key]=x.value);const f=$("#settingsForm");if(f){f.minimum_order_amount.value=Number(siteSettings.minimum_order_amount||0);f.shipping_cost.value=Number(siteSettings.shipping_cost||0);f.shipping_enabled.checked=siteSettings.shipping_enabled!=="false";f.theme_primary.value=siteSettings.theme_primary||"#FF5A1F";f.theme_bg.value=siteSettings.theme_bg||"#F7F7F8";}}
...
async function saveSettings(e){
 ...
 try{
  ...
  const values={minimum_order_amount:String(Math.max(0,Number(f.get("minimum_order_amount")||0))),shipping_cost:String(Number(f.get("shipping_cost")||0)),shipping_enabled:f.get("shipping_enabled")?"true":"false",theme_primary:f.get("theme_primary")||"#FF5A1F",theme_bg:f.get("theme_bg")||"#F7F7F8"};
...
function renderProducts(){
 ...
 if(!list.length){box.innerHTML='<div class="empty-state">محصولی وجود ندارد.</div>';return}
 box.innerHTML=list.map((p,i)=>`<div class="admin-product" draggable="${q?"false":"true"}" data-product-id="${p.id}" title="${q?"برای جابه‌جایی، جستجو را پاک کنید":"برای جابه‌جایی بکشید و رها کنید"}">
   ...
   <div class="admin-stock">موجودی: ${num(p.stock)} · ${p.is_visible===false?"مخفی":"منتشر"}</div>
...
async function saveProductOrder(){
 ...
 [...document.querySelectorAll("#adminProducts .admin-product")].forEach((el,i)=>{const n=el.querySelector(".drag-handle span");if(n)n.textContent=num(i+1)});
...
async function deleteOrder(id,number){
 if(!confirm(`فاکتور ${number} و تمام اقلام آن برای همیشه حذف شود؟`))return;
 const {error}=await sb.from("orders").delete().eq("id",id);
 if(error){toast("حذف سفارش انجام نشد: "+error.message);return}
 allOrders=allOrders.filter(o=>o.id!==id);renderOrders();renderStats();toast("فاکتور و تمام جزئیات سفارش حذف شد");
}

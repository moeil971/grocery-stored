...
async function loadSettings(){const {data,error}=await sb.from("site_settings").select("*");if(!error)(data||[]).forEach(x=>siteSettings[x.key]=x.value);const f=$("#settingsForm");if(f){f.shipping_cost.value=Number(siteSettings.shipping_cost||0);f.shipping_enabled.checked=siteSettings.shipping_enabled!=="false";f.theme_primary.value=siteSettings.theme_primary||"#FF5A1F";f.theme_bg.value=siteSettings.theme_bg||"#F7F7F8";}}
...
async function saveSettings(e){
 ...
 try{
  ...
  const values={shipping_cost:String(Number(f.get("shipping_cost")||0)),shipping_enabled:f.get("shipping_enabled")?"true":"false",theme_primary:f.get("theme_primary")||"#FF5A1F",theme_bg:f.get("theme_bg")||"#F7F7F8"};
